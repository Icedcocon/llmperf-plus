...
def run_token_benchmark_for_concurrency(
    llm_api: str,
    model: str,
    test_timeout_s: int,
    ...
    max_num_completed_requests = concurrency * 4
...
def generate_csv_report(results, results_dir, model, mean_input_tokens, mean_output_tokens):
    ...
    for concurrency in concurrencies:
        result = results[concurrency].get('results', {})  # Get data from 'results' key
        print(f"\nProcessing results for concurrency {concurrency}")
        print(f"Result keys: {result.keys()}")
        print(f"Result content: {result}")
        
        try:
            # Get metric data
            inter_token_lat = result.get(common_metrics.INTER_TOKEN_LAT, {})
            print(f"inter_token_lat: {inter_token_lat}")
            
            # Format numeric values to 4 significant digits
            def format_value(value):
                if isinstance(value, (int, float)):
                    return f"{value:.4g}"
                return value

            data['inter_token_latency_s'].append(format_value(inter_token_lat.get('mean', 'N/A')))
            data['token/s'].append(format_value(1 / inter_token_lat.get('mean', float('inf')) if inter_token_lat.get('mean') else 'N/A'))
            data['ttft_s'].append(format_value(result.get(common_metrics.TTFT, {}).get('mean', 'N/A')))
            data['end_to_end_latency_s'].append(format_value(result.get(common_metrics.E2E_LAT, {}).get('mean', 'N/A')))
            data['request_output_throughput_token_per_s'].append(format_value(result.get(common_metrics.REQ_OUTPUT_THROUGHPUT, {}).get('mean', 'N/A')))
            data['number_input_tokens'].append(format_value(result.get(common_metrics.NUM_INPUT_TOKENS, {}).get('mean', 'N/A')))
            data['number_output_tokens'].append(format_value(result.get(common_metrics.NUM_OUTPUT_TOKENS, {}).get('mean', 'N/A')))
            data['Overall Output Throughput'].append(format_value(result.get(common_metrics.OUTPUT_THROUGHPUT, 'N/A')))
            data['Completed Requests Per Minute'].append(format_value(result.get(common_metrics.COMPLETED_REQUESTS_PER_MIN, 'N/A')))
        except Exception as e:
            print(f"Error processing results for concurrency {concurrency}: {str(e)}")
            print(f"Complete result data: {result}")
            for metric in metrics:
                data[metric].append('N/A')
    
    df = pd.DataFrame(data, index=concurrencies)
    
    filename = f"{model}_{mean_input_tokens}_{mean_output_tokens}_concurrency_report.csv"
...
def main_test_concurrency():
    ...
    for concurrency in concurrencies:
        ...
        summary = run_token_benchmark_for_concurrency(
            llm_api=args.llm_api,
            model=args.model,
            test_timeout_s=args.timeout,
            mean_input_tokens=args.mean_input_tokens,
    ...
    generate_csv_report(results, args.results_dir, args.model, args.mean_input_tokens, args.mean_output_tokens)

if __name__ == "__main__":
    # Usage
    #  OPENAI_API_KEY="xxxx" OPENAI_API_BASE="http://192.168.5.3:9998/v1" python token_benchmark_ray.py --model qwen-chat --mean-input-tokens 1024 --stddev-input-tokens 128 --mean-output-tokens 128 --stddev-output-tokens 128 --timeout 600 --results-dir /workspace/dev/results --llm-api openai --additional-sampling-params "{}"
    main_test_concurrency()