...
from llmperf.ray_clients.vllm_generate_client import VLLMGenerateClient


SUPPORTED_APIS = ["openai", "anthropic", "litellm", "vllmgenerate"]
...
def construct_clients(llm_api: str, num_clients: int) -> List[LLMClient]:
    ...
    elif llm_api == "vllmgenerate":
        clients = [VLLMGenerateClient.remote() for _ in range(num_clients)]
